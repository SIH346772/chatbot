...
l_df = pandas.read_csv(f"embs0.csv")
embs = []
for r1 in range(len(l_df.embedding)): # Changing the format of the embeddings into a list due to a parsing error
    e1 = l_df.embedding[r1].split(",")
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=8000, debug=False) # Runs the ChatBot on port:8000 (you can use a different one)